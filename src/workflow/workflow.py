...
class EmailWorkflow:
    def __init__(self, logger: logging.Logger):
        ...
        def input_parser_router(state: AgentState) -> Union[str, END]:
            """
        ...
        def validation_router(state: AgentState) -> str:
            ...
            - If validation_report contains FAIL, retry draft_writer
            - Otherwise proceed to memory + END
            """
            report = state.get("validation_report", "")
            if isinstance(report, str) and report.upper().startswith("FAIL"):
                state["retry_count"] += 1
    ...
    # ----------------------------------------------------------------------
    async def run_query(self, user_input: str) -> Dict[str, Any]:
        ...
        return {
            "draft": final_state.get("personalized_draft")
